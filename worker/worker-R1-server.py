...
ap = PlainTextAuthProvider(username='cassandra', password='changeme')
...
while True:
    ...
    redisClient.lpush("logging", "worker 1 consumer message get " + str(datetime.now()))
    user_id = msg.key()
    ...
    redisClient.lpush("logging",  "insert complete, " + str(datetime.now()))
    ######### cassandra ##########
